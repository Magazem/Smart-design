...
## Q.8 GitHub search (Design Researcher, 2026-09-24; sole GitHub-API user; adds to Q.1 row "GitHub `devis/quotation latex` generators")

Queries (one call each, 8 s apart, `curl -s -A smart-design-research`, sort stars desc, per_page 100): `angebot latex` (total_count 0); `angebot template` (total_count 0); `angebot typst` (total_count 0); `devis latex` (total_count 3); `devis template` (total_count 141); `devis typst` (total_count 0); `estimate template latex` (total_count 0); `quotation template latex` (total_count 1); `quotation typst` (total_count 1); `quote template latex` (total_count 3); `\r_cotizacion latex` (total_count 0); `\r_devis facture latex` (total_count 2); `\r_estimate latex` (total_count 29); `\r_kostenvoranschlag` (total_count 1); `\r_offer letter template latex` (total_count 0); `\r_offerte latex` (total_count 1); `\r_orcamento latex` (total_count 0); `\r_presupuesto latex` (total_count 0); `\r_preventivo latex` (total_count 1); `\r_quotation latex` (total_count 8); `\r_quote invoice latex` (total_count 3); `\r_quote typst` (total_count 5). Raw hits were dominated by noise ("devise" Rails templates for `devis template`, statistics "estimate" papers, Bible-quotation styles). Distinct on-topic candidates (quote/devis/offer templates or generators): **12** (OrderAndCh4oS/Invoicing, sylrouss/facturation, gerfru/LaTex-Offer, Pierre-Sassoulas/devis, ProgrammingSam/quotation-generator, jappeace/offertex, Lukwos/QuotationGenerator, leopoussines/template_devis, aittakkasarah-jpg/Devis_Opti-clean, mabellou/Template-Gestapp-devis, kaat0/typst-faktura, Paul-Veber/TypstQuoteScript). Already counted: Tiefseetauchner/TiefLetter (QP:001), DeVinci-FabLab/Invoice-templates (QP:002), NikosAlexandris/invoice_el (QP:003), (none of them was among the repos these queries returned).

Codeable: **1 new item**, ProgrammingSam/quotation-generator (committed `example.pdf`, a "Devis n. 2019/1", page 1 rendered at 80 dpi in temp). Not codeable: OrderAndCh4oS/Invoicing (README GIF of a console session, no page), leopoussines/template_devis (a 266x150 dark web-slide thumbnail, not a quote page, C2), and 9 with no preview file (README/root/first-level dirs checked).

| id | columns | heading | body | colour | header | rules/boxes | density | totals | table rules | adm | measurements / notes |
|---|---|---|---|---|---|---|---|---|---|---|---|
| QP:004 | 1 | serif | serif | one-accent | ruled | rules | standard | right-bottom | header-and-total | y | Title "Devis n. : 2019/1" bold serif with a bold subtitle; the ProgrammingSam wordmark is a logo (excluded, B1b); sender address is right-aligned above the title. A full-width red rule sits directly below the title/subtitle block (about 90% of live width, no text between, gap < 4 lines), so `ruled` precedes plain-centered (82a-general A.2 step 3). Red rules x4 (under the subtitle, above and below the table header, under the item row) plus a double red rule above "Total TTC" = one hue cluster, >= 2 elements (B5) -> one-accent; the pink page frame in the rendering is a capture border and is ignored (A3 of 82a-cv). Table: two horizontal red rules + a double rule before the total, no verticals (`header-and-total`); the amounts sit in the right column, and the totals rows (Total HT, TVA 21%, Total TTC) are left-aligned labels with right-column values (`right-bottom`). One serif family; no A1-A8 hit; C7 none. |

### Q.8.1 Recount
- **On-topic codeable quote items: 3 + 1 = 4**. This is still < 10, so the A1 pool is not formed. Quote keeps no corpus and no k/N (82 section 2; 82a C15). The A5 twin borrowing (Q.5) is unchanged.
- Presence tally, all 4 items: `1|serif|mono|plain-left` (QP:001, QP:002), `1|serif|one-accent|plain-left` (QP:003), `1|serif|one-accent|ruled` (QP:004). Admissible 4/4.
- Second-coder ids: QP:001, QP:002, QP:003, QP:004 (`quote-items.csv`, 4 rows); the C10 sample formula gives all 4 (`max(min(10,4), ceil(1.0)) = 4`).
- Bias: the GitHub quote/devis surface is tiny and francophone/German; most repos are generators or apps, not fixed designs, and most ship no preview page.
