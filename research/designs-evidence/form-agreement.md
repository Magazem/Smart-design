...
- `colour` from `form-recode-82ag.md`: 4 override rows found (2 tables — a `(r1)`/`(r2ag)`
  ...
  | FMA:004 | *(not extracted — see note)* | one-accent, BORDERLINE |
  ...
  numeric value is lost — `mono`/`one-accent, BORDERLINE` are exactly the corpus's Fm.8.3 values,
  independently re-derived by the recoder and confirmed unchanged.
...
| colour | identity | 4 | 0 | 0.0000 | 0.0000 | **FAIL** |
...
| FMA:004 | colour | one-accent, BORDERLINE | one-accent |
...
the second coder counts each authority's link colour (`one-accent`) where the first coder
...
**FAIL** — identity features `columns` (A_f=0.00), `heading` (A_f=0.50) and `colour` (A_f=0.00,
post-recode) are all below the 0.80 falsifier-gate threshold. Only `field_style` (1.00) passes
among the identity set.
