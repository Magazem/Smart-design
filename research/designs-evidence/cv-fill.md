...
## 9. Re-run 2026-09-25 under R7 (research/92-review-r7-cv-fill.md, research/82a-r7-rulings.md)

**This section supersedes §1-§5 wherever they differ** (12 designs, rank 3, Georgia rows, palette prose).
The cv fill was regenerated by `fill_family.py --write` from `fill-specs/cv.json`, which now holds only
the authored wording (names, Best-for, new style rows, `own_archetype`, `pending`) and **0 overrides**;
ranks, evidence classes, palette / typeface / style keys, bias terms and provenance shares are the
engine's. The generated log is `cv-fill-log.md` (engine ranking, plan, proposals, blanked tokens).

Engine changes applied: R7-1 admissible-only shares; R7-2 medium rule; R7-3 literal palette order (no
82a-clarifications-6); R7-4 `--check`/`--write` fail on spec != engine without a ratified override;
R7-5 family-prefix reuse + photo contradiction + body class; R7-6 corpus tie order (level, N, id);
R7-7 cap 10, default always shipped, identity-duplicate retirement; R7-8 blank contradicting tokens;
R7-9 declared-font branch (cv corpora carry no declared fonts: logged "not evaluable"); R7-11 palette
classification tests; F17 library rows read once, and a re-run no longer sees its own earlier rows.

### Ranks, old vs new

| design | old rank (class) | new rank (class) | why |
|---|---|---|---|
| cv-serif-plain-centered | 1 ranked | 1 ranked | unchanged |
| cv-eu-europass | 2 authority | 2 ranked | F10: merged design keeps its authority row, class is `ranked` |
| cv-sans-mono-split | 4 | 3 | R7-1: ruled loses 2 inadmissible items (6 -> 4), the 0.050 tie is broken by GH share |
| cv-ats-strict | 5 | 4 | same tie |
| cv-sans-accent-ruled | 3 | 5 | R7-1: combined 0.0750 -> 0.0500, provenance `0.150 (6/40)` -> `0.100 (4/40)` |
| cv-serif-accent-plain-left | 6 | 6 | unchanged |
| cv-sans-accent-plain-centered | 7 | 7 | unchanged |
| cv-serif-mono-split | 8 | 8 | unchanged |
| cv-sans-accent-split | 9 | not shipped | R7-7: cap 10 is total; displaced from the last ranked slot by the doctype default |
| cv-us-uk-designed | 10 authority | 9 ranked | R7-7: default whose archetype ranks 13th takes the last ranked slot |
| cv-academic | 11 convention | 10 convention | unmatched default, counted inside the cap of 10 |
| cv-dach-tabular | 12 convention | not shipped | R7-10 pending (render check not run); also an identity duplicate of cv-academic (`1|serif|mono|plain-left`) under R7-7 |

Total shipped 10 (was 12). Interpretation to confirm: an unmatched default (cv-academic) counts inside
the cap, so ranked slots = 9; if the orchestrator wants it exempt (11 total) the archetype
`1|sans|one-accent|split` would return at rank 9 and cv-us-uk-designed would take rank 10.

### Fill picks, old vs new

| row | field | old | new |
|---|---|---|---|
| cv-sans-accent-ruled | Style Key | cv-europass (imported the Europass section order and language grid) | cv-sans-accent-ruled (new row; R7-5/R7-8 reject the region-specific row) |
| cv-serif-accent-plain-left | Style Key | cv-restrained | cv-academic-plain (family-prefixed reuse candidate first in byte order) |
| cv-serif-mono-split | Style Key | cv-sans-accent-plain-centered (a centred-name row for a split header) | cv-serif-mono-split (new row; the family's own earlier rows are hidden on re-run) |
| ranks 1, 6, 8 | Typeface Key | safe-serif-times (already corrected 2026-09-25) | safe-serif-times, now produced by the engine's medium rule |
| all palette keys | Palette Key | carbon-mono / atlassian-ink | unchanged: fetched authority before search-corroborated is the literal R7-3 order |
| accent rows | Palette Bias Terms | "monochrome, ink on white, no color-coded meaning" | "no color-coded meaning" (`monochrome`, `ink on white` blanked, 3 rows) |
| lib-roboto rows | Typeface Bias Terms | "safe stack, ubiquitous, no embedding required" | blank (3 rows) |

Open point for the orchestrator: `cv-serif-accent-plain-left` now reuses `cv-academic-plain`, whose
checklist says "Do not enforce a page limit; List publications ..." -- legal under section 8 (no
column / photo / region contradiction) but odd for a generic CV. I did not tune the rule; adding
"academic" to the R7-8 token list would send it to a new row.

### R7-10

`cv-dach-tabular` is marked `pending` in the spec and is **not shipped**: I did not run the section 12
F6 ATS extraction render check on its label/content body table.

### Checks

`fill_family.py --family cv --check`: 0 violations, 4 x same; gate OK; skill pytest 328 passed;
test_fill_family 57 passed; research/p65 10 passed.

### 9a. Update after 82a-clarifications-6 was RATIFIED (2026-09-25)

- **R6 cap**: every doctype default counts inside the 10. Engine: ranked slots k = the largest k such that
  k + (matched defaults outside the first k) + (unmatched L3/defaults/L4) <= 10. For cv k = 8; the shipped list
  is unchanged (ranks 9 = cv-us-uk-designed, 10 = cv-academic), `cv-sans-accent-split` stays outside.
- **R7 audience tokens**: a style row naming academic / publications / page limit (and Europass, DACH, ...) is
  not reusable for a design that is not that audience (`FAMILY_SPECIFIC`, tested). `cv-serif-accent-plain-left`
  therefore no longer reuses `cv-academic-plain`; the engine's next legal reuse is the generic `cv-restrained`
  (its old pick), not a new row. If a new row is wanted for the header placement (R4 guidance), it needs an
  `override` citing a ratified rule -- say so and I will author it.
- **R2** is ratified: log labels read "rule R2, research/82a-clarifications-6.md". `_override_ok` accepts the
  file (a `RATIFIED` status line wins over the historical "draft" wording later in the head; tested).
